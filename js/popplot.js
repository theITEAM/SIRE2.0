...
{
	var i, j, tx, val, bin=[], wei=[], range = 10, novari, cc = 5, c, stat=[], poptramin, poptramax;
	...
	if(groupsel == "All"){
		poptramin = infres.tmin; poptramax = infres.tmax; 
	}
	else{
		z = 0; while(z < infres.triallist.length && infres.triallist[z] != groupsel) z++;
		if(z == infres.triallist.length) alertp("Problem EC15.");
		poptramin = infres.trialtmin[z]; poptramax = infres.trialtmax[z];
	}

	if(xaxisauto == 1){
		poptmin = poptramin; poptmax = poptramax;
		axxmin = poptmin; axxmax = poptmax;
		if(axxmax > 0 && axxmin > 0){ if(axxmin < 0.3*axxmax) axxmin = 0;}
		axxmax += 0.001*(axxmax-axxmin);
	}
	else{
		axxmin = xaxisfixmin; axxmax = xaxisfixmax;
		poptmin = axxmin; if(poptmin < poptramin) poptmin = poptramin;
		poptmax = axxmax; if(poptmax > poptramax) poptmax = poptramax;
	}
...
function popplotinit()                                                                         // Initialises population plot
{
	...
	indcalc=[];
	for(i = 0; i < N; i++){
		if((groupsel == "All" && infres.inddata.ind[i].trial != -1)  || infres.inddata.ind[i].trial == groupsel) indcalc.push(i);
	}
	
	npop = 0;
	if(modtype == SIR) kmax = 3; else kmax = 2;
