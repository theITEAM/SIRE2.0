...
function converttoobs(ty)                                                         // Converts data sources into individual measurements
{
	...
	for(d = 0; d < da.length; d++){
		...
		case "snp": case "fixed": case "fixedcat": case "ped": case "breval":
	...
	if(isdata("trial") > 0){
		N = inddata.nindtotal;
		for(i = 0; i < N; i++){
			stt = inddata.ind[i].trial;
			if(stt != "."){
				j = 0; while(j < triallist.length && stt != triallist[j]) j++;
		...
		for(i = 0; i < N; i++){
			...
			if(j == triallist.length){
				if(stt == ".") inddata.ind[i].trialnum = -1;
				else alertp("Error EC3");
			}
			else inddata.ind[i].trialnum = (j+1);
		}
	}
