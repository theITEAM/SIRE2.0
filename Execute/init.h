...
void init()                                                       // Initialises variables before posterior sampling can begin
{
  ...
  trialind.clear(); trialind.resize(Z); 
  for(j = 0; j < N; j++){ z = indtrial[j]; if(z >= 0) trialind[z].push_back(j);}
  ...
  for(j = 0; j < N; j++){
    ntr_I[j] = 0; nac_I[j] = 0; jumpI[j] = 0.5; if(jumpI[j] > Itmax[j]-Itmin[j]) jumpI[j] = Itmax[j]-Itmin[j]; 
    ntr_R[j] = 0; nac_R[j] = 0; jumpR[j] = 0.5; if(jumpR[j] > Rtmax[j]-Rtmin[j]) jumpR[j] = Rtmax[j]-Rtmin[j];
  ...
  for(j = 0; j < N; j++){ if(indtrial[j] < 0) infstat[j] = -1;}
  ...
  Pri = prior(); if(Pri <= -big/2) emsg("The prior is not satisfied");
