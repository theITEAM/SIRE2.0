    ...
    for(lo = 0; lo < 6; lo++){
      if(mod == SIR || (mod == SI && (lo == 0 || lo == 1 || lo == 3))){
        switch(lo){
          ...
          case 1: te = vara_gf; vara_gf += normal(0,jumpvara_gf); dPri = priorind(prior_vara_gf,vara_gf)-priorind(prior_vara_gf,te); break;
          case 2: te = vara_gr; vara_gr += normal(0,jumpvara_gr); dPri = priorind(prior_vara_gr,vara_gr)-priorind(prior_vara_gr,te); break;
          case 3: te = vara_ff; vara_ff += normal(0,jumpvara_ff); dPri = priorind(prior_vara_ff,vara_ff)-priorind(prior_vara_ff,te); break;
          case 4: te = vara_fr; vara_fr += normal(0,jumpvara_fr); dPri = priorind(prior_vara_fr,vara_fr)-priorind(prior_vara_fr,te); break;
