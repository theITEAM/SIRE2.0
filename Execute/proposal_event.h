...
void propevent()                                                   // Proposes event changes along the timelune
{
  ...
  for(j = 0; j < N; j++){
    ...
    if(geffon == 1){ z = indtrial[j]; if(z >= 0) val_g += geff_g[z];}
...
void shiftI(long e)                                                        // Moves an infection event
{
  ...
  if(noIinrangest[j] == 1){ z = indtrial[j]; if(z < 0) emsg("prob"); if(tnew > obstmin[z] && tnew < obstmax[z]) return;}
...
void shiftR(long e)                                             // Moves a recovery event
{
  ...
  if(noRinrangest[j] == 1){ z = indtrial[j]; if(z < 0) emsg("Prob"); if(tnew > obstmin[z] && tnew < obstmax[z]) return;}
