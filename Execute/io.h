...
{
  long i, ist, fl;
  long z, gen, fi, j, jj, zcol, SNPcol, Itcol, Rtcol, enttcol, leatcol, statecol, qgcol, qfcol, qrcol, pr, pr2, copy, flag, Ipos, noRinrange, noIinrange, diat;
  ...
  nfi = 0; randon = 0;
  zcol = -1; SNPcol = -1; Itcol = -1; Rtcol = -1; enttcol = -1; leatcol = -1; statecol = -1; qfcol = -1; qgcol = -1; qrcol = -1;
  ...
  q_g_bv.clear(); q_f_bv.clear(); q_r_bv.clear();
   
  Z = 0; envon = 0; geffon = 0; domon = 1;
  ...
  for(child = root->FirstChild(); child; child = child->NextSibling()){
    ...
    if(s1.compare("datatable") == 0){  // model
      if(exi(child,"group") == true){
        zcol = long(getnum(child,"group"))-1; if(zcol < 0) emsg("Problem with contact group column number");
      }
      else{
      ...
      if(exi(child,"qg") == true){
        qgcol = long(getnum(child,"qg"))-1;
      }
      
      if(exi(child,"qf") == true){
        qfcol = long(getnum(child,"qf"))-1;
      }
			
			if(exi(child,"qr") == true){
        qrcol = long(getnum(child,"qr"))-1;
      }
      ...
      for(child2 = child->FirstChild(); child2; child2 = child2->NextSibling()){
        ...
        while(getline(ss, li)){
          ...
          if(sti.size() >= 3){
            if(zcol >= 0){
							if(sti[zcol].c_str() == ".") z = -1;
							else z = atoi(sti[zcol].c_str())-1;
						}
						else z = 0;
              ...
              if(z >= 0 && sti[Rtcol] != "."){
                if(sti[Rtcol] == "no"){
                  ...
                  if(Rti < trialtmin[z] || Rti > trialtmax[z]){
										emsg("Recovery time outside observation period");
									}
                  Rti += 0.00001*ran();
                  Rtmi = Rti; Rtma = Rti;
            ...
            for(fi = 0; fi < nfi; fi++) Xt[fi] = atof(sti[Xcols[fi]].c_str());
						...
						if(qgcol >= 0) q_g_bv.push_back(atof(sti[qgcol].c_str()));
						if(qfcol >= 0) q_f_bv.push_back(atof(sti[qfcol].c_str()));
						if(qrcol >= 0) q_r_bv.push_back(atof(sti[qrcol].c_str()));
  ...
  initevents();
	...
	/*
	for(j = 0; j < N; j++){ // captures DS2
		if(indtrial[j] >= 0 && Itmin[j] != -big && Rtmin[j] != -big){ 
			Itmin[j] = 0; Itmax[j] = Rtmin[j];
		}
	}  
	 */
  ...
  for(child = root->FirstChild(); child; child = child->NextSibling()){
    ...
    if(s1.compare("Ainv") == 0){  // inverse relationship matrix
      ...
      for(child2 = child->FirstChild(); child2; child2 = child2->NextSibling()){
		...
		if(s1.compare("Ainv_nonzero") == 0){  // inverse relationship matrix
			randon = 1;
			for(child2 = child->FirstChild(); child2; child2 = child2->NextSibling()){
				s2 = child2->Value();
				stringstream ss (s2);
				...
				while(getline(ss, li)){
					if(li.length() > 3){
						...
						ss2 >> j >> jj >> val;
						if(j != jj){
							Ainvlist[j].push_back(jj); Ainvlistval[j].push_back(val); nAinvlist[j]++;
							Ainvlist[jj].push_back(j); Ainvlistval[jj].push_back(val); nAinvlist[jj]++;
		...
		if(s1.compare("A_nonzero") == 0){  // inverse relationship matrix
			randon = 1;
			
			A.resize(N); Ainv.resize(N);
			for(j = 0; j < N; j++){
				A[j].resize(N); Ainv[j].resize(N);
				for(jj = 0; jj < N; jj++) A[j][jj] = 0;
			}
	...
	for(child = root->FirstChild(); child; child = child->NextSibling()){
    s1 = child->Value();
    if(s1.compare("prediction_accuracy") == 0){  
			PredAcc pa;
			pa.name = get(child,"name");
			string inds = get(child,"ind");
			vector <string> indstr = split(inds,',');
			for(j = 0; j < indstr.size(); j++){
				if(indstr[j] != ""){
					for(i = 0; i < indid.size(); i++){ if(indid[i] == indstr[j]) break;}
					if(i == indid.size()){
						stringstream ss; ss << "Cannot find individual '" << indstr[j] << "' from the prediction accuracy.";
						emsg(ss.str());
					}
					pa.ind.push_back(i);
				}
			}
			predacc.push_back(pa);
		}
	}
  ...
  for(child = root->FirstChild(); child; child = child->NextSibling()){
    ...
    if(s1.compare("prior") == 0){    // priors
      if(exi(child,"parameter")){
        ...
        if(get(child,"parameter") == "Ψ_gg") pr = prior_vare_gg;
        if(get(child,"parameter") == "Ψ_ff") pr = prior_vare_ff;
        if(get(child,"parameter") == "Ψ_rr") pr = prior_vare_rr;
        if(get(child,"parameter") == "Ψ_gf") pr = prior_vare_gf;
        if(get(child,"parameter") == "Ψ_gr") pr = prior_vare_gr;
        if(get(child,"parameter") == "Ψ_fr") pr = prior_vare_fr;
...
void initevents()                                         // Generates the initial set of infection and recovery times
{
  ...
  double nav, av, av2, Rtsd, Itsd, nRtsd, nItsd, sd, sd2, Rti, Iti, tmi, tma, tmi2, tma2, val, Sp, sum, mi, ma, tprob, pval;
  vector <EVV> evl;
  vector <double> Itav;
  ...
  for(j = 0; j < N; j++){                                                              // infection times
	...
	if(Itmin[j] == Itmax[j]) It[j] = Itmin[j];
	  ...
	  sd2 = sd; if(sd2 > Itmax[j]-Itmin[j]) sd2 = Itmax[j]-Itmin[j];
	  
	  loop = 0; do{ It[j] = Itmax[j] + log(ran())*sd2; loop++;}while(loop < 1000 && (It[j] < Itmin[j] || It[j] > Rtmax[j]));
	 
	  
	  if(loop == 1000){ cout << Itmin[j] << " " << Itmax[j] << " minmax\n"; stringstream ss; ss << "Cannot find initial condition for individual " << indid[j]; emsg(ss.str());}
	}
      }
      else{
	It[j] = -big;
  ...
  trialunknownI.resize(Z);
  for(j = 0; j < N; j++){
    z = indtrial[j];
    if(z >= 0){
      if(Iposst[j] == unknownI){ unknownIfl = 1; trialunknownI[z].push_back(j);}
    }
...
{                                 
  ...
  string split;                                      
  istringstream ss(s);                               
  while (getline(ss, split, delimiter)) splits.push_back(split);                                                  
  return splits;                                           
}
